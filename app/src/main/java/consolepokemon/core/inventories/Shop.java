...
public class Shop extends Inventory
{
	...
	public boolean buy(HumanTrainer trainer, int index, int buyCount){
		...
		Item ware = super.getItem(index);
		if(ware == null){
			Log.v("购买错误，无此商品.");
			return false;
		}
		int laterWareCount = ware.count-buyCount;
		if(laterWareCount < 0){
			Log.v("购买失败，商店没有那么多存货.");
			return false;
		}
		int price = getPrice(ware) * buyCount;
		int laterCoin = trainer.coin - price;
		if(laterCoin < 0){
			Log.v("购买失败，你的晶币不够.");
			return false;
		}
	...
	public void show(){
		...
		for(int i=0;i<items.size();i++){
			Item ware = super.getItem(i);
			int price = getPrice(ware);
